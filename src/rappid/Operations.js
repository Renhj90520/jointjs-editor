...
export class ElementOperation {
  ...
  startResizing(a) {
    this.startBatch(),
      (this._flip = [1, 0, 0, 1, 1, 0, 0, 1][
        Math.floor(
          joint.g.normalizeAngle(this.options.cellView.model.get("angle")) / 45
        )
      ]);
  }
  ...
  startCloning(b, c, d) {
    ...
    if (!(cloneView instanceof a.dia.Cell))
      throw new Error('ui.Halo: option "clone" has to return a cell.');
  ...
  centerElementAtCursor(element, b, c) {
